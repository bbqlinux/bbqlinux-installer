...
class InstallerEngine(QtCore.QThread):
        ...
        for partition in setup.partitions:                    
            if(partition.format_as is not None and partition.format_as != "" and partition.format_as != "None"):                
                ...
                if partition.format_as == "swap":
                    ...
                    if (partition.format_as == "ext2") or (partition.format_as == "ext3") or (partition.format_as == "ext4") or (partition.format_as == "ext4dev"):
                        cmd = "mkfs.%s -qF %s" % (partition.format_as, partition.partition.path)
                        ...
                        cmd = "mkfs.%s -q %s" % (partition.format_as, partition.partition.path) # works with bfs, btrfs, jfs, minix, msdos, ntfs
    ...
    def install(self, setup):
        ...
        try:
            ...
            if(os.path.exists("/target/etc/systemd/system/etc-pacman.d-gnupg.mount")):
                self.do_run_in_chroot("rm -f /etc/systemd/system/etc-pacman.d-gnupg.mount")
            if(os.path.exists("/target/etc/systemd/system/pacman-init.service")):
                self.do_run_in_chroot("rm -f /etc/systemd/system/pacman-init.service")
